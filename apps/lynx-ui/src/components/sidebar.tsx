...
function SidebarProvider({
  ...
  // Mobile gestures: Swipe from left and Pinch-in to open
  ...
  React.useEffect(() => {
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      if (e.touches.length === 1) {
        gestureState.current.touchStartX = e.touches[0].clientX;
      } else if (e.touches.length === 2) {
        // Track average start position for 2-finger swipe
        gestureState.current.touchStartX =
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      if (!openMobile) {
        // Opening gestures (Requires 2 fingers)
        if (e.touches.length === 2) {
          const currentTouchX =
            (e.touches[0].clientX + e.touches[1].clientX) / 2;
          const deltaX = currentTouchX - gestureState.current.touchStartX;
          const currentDistance = Math.hypot(
            e.touches[0].clientX - e.touches[1].clientX,
            e.touches[0].clientY - e.touches[1].clientY,
          );

          // 1. Swipe from left edge (both fingers started near edge)
          const isFromEdge = gestureState.current.touchStartX < 80;
          if (isFromEdge && deltaX > 70) {
            setOpenMobile(true);
            gestureState.current.gestureTriggered = true;
            return;
          }

          // 2. Pinch in: distance decrease by more than 50px
          if (
            gestureState.current.initialDistance > 0 &&
            currentDistance - gestureState.current.initialDistance < -100
          ) {
            setOpenMobile(true);
            gestureState.current.gestureTriggered = true;
          }
    ...
    const handleTouchEnd = () => {
      // Reset lockout when all fingers are lifted
      gestureState.current.gestureTriggered = false;
    };
