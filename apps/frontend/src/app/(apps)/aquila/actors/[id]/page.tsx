...
export default function ActorPage() {
  ...
  const { data: favStatus, mutate: mutateFav } = useSWR<{ favorited: boolean }>(
    id && session?.accessToken
      ? [
          `${process.env.NEXT_PUBLIC_API_URL}/favorites/STAFF/${id}/status`,
          session.accessToken,
        ]
      : null,
    fetcher,
  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (isFavorited) {
        await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/favorites/STAFF/${id}`,
          {
            method: "DELETE",
  ...
  const displayName = useMemo(() => {
    if (!actor) return t("aquila.actorDetails");
    return actor.namePrimary || t("aquila.unknownActor");
  }, [actor, t]);
                ...
                {hasNoRoles ? (
                  ...
                  Object.entries(rolesByType).map(([mediaType, roles]) => (
                    <RolesSection
                      ...
                      roles={roles}
                      routePrefix={
                        MEDIA_TYPE_ROUTE[mediaType] ??
                        `/aquila/${mediaType.toLowerCase()}`
                      }
                    />
                  ))
                )}
  ...
  return (
    <motion.div variants={itemVariants} className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {roles.map((role, idx) => {
          const isStaffRole = !role.characterName;
          const displayRole = role.customRole || role.role || "Staff";
          ...
          if (isStaffRole) {
            return (
              <Link
                ...
                className="flex items-center gap-3 p-3 bg-card/45 border border-border/30 backdrop-blur-md rounded-xl overflow-hidden hover:border-border/50 hover:bg-accent/5 transition-all duration-300 group/media"
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1">
                    <Badge
                      variant="secondary"
                      className="text-[10px] px-2 py-0.5 font-medium rounded-lg"
                    >
                      {displayRole}
                    </Badge>
                  </div>
          ...
          const charName = role.characterName;
