...
export async function GET(
  ...
  if (!session?.accessToken) {
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_AQUILA}/connections?error=unauthorized`,
    );
  }
  ...
  if (!provider || !code) {
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_AQUILA}/connections?error=invalid_request`,
    );
  }
  ...
  if (!clientId || !clientSecret) {
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_AQUILA}/connections?error=missing_config`,
    );
  }

  try {
    const redirectUri = `${process.env.NEXT_PUBLIC_AQUILA}/api/connections/${providerId}/callback`;
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_AQUILA}/connections?success=true`,
    );
  } catch (error) {
    console.error(`OAuth callback error [${providerId}]:`, error);
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_AQUILA}/connections?error=oauth_failed`,
    );
  }
}
