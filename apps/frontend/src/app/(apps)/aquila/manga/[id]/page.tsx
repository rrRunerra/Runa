...
export default function MangaDetailsPage() {
  ...
  // Shared dialog form (used for both Add and Edit dialogs)
  const renderDialogForm = (isEdit: boolean) => (
    <DialogContent className="sm:max-w-[700px] p-0 overflow-hidden bg-popover border-border text-popover-foreground [&>button]:text-foreground [&>button]:z-60 [&>button]:hover:text-muted-foreground">
      <DialogTitle className="sr-only">
        {isEdit ? "Edit Manga Entry" : "Add Manga to List"}
      </DialogTitle>
        ...
        <div className="grid grid-cols-6 gap-x-6 gap-y-4">
            ...
            <Select
              ...
              onValueChange={(val) => {
                ...
                if (val === "COMPLETED" && !finishDate) {
                  ...
                  if (manga.chapters && !chapters)
                    setChapters(manga.chapters.toString());
                }
                if (val === "READING" && !startDate) {
          ...
          <div className="col-span-2 flex flex-col gap-2">
            ...
            <div className="flex bg-background border border-input rounded-md overflow-hidden">
              <Input
                ...
                onChange={(e) => {
                  let val = e.target.value;
                  if (manga.chapters && Number(val) >= manga.chapters) {
                    val = manga.chapters.toString();
          ...
          <div className="col-span-6 flex flex-col gap-2 mt-2">
            <div className="flex items-center space-x-2">
              <Checkbox
                id={`update-connection-${isEdit ? "edit" : "add"}`}
              ...
              />
              <Label
                htmlFor={`update-connection-${isEdit ? "edit" : "add"}`}
      ...
      >
        <DialogContent className="sm:max-w-[425px] bg-popover border-border">
          <DialogHeader>
            ...
            <DialogDescription>
              Search for this manga to link it.
            </DialogDescription>
          </DialogHeader>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {result.chapters ? `${result.chapters} ch` : "? ch"}
                      </p>
                    </div>
                  </div>
                ))}
