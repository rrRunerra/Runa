...
export default function UserMangaPage({ initialData }: { initialData?: any }) {
  ...
  useEffect(() => {
    ...
    setHasMore(true);
    fetchMangaList(undefined, true, controller.signal);
    return () => {
      controller.abort();
    };
  }, [
            ...
            <div className="flex flex-col gap-4 w-full bg-card/20 backdrop-blur-xl border border-border/40 p-4 rounded-2xl shadow-xl">
              ...
              {activeList !== "Similar" && activeList !== "Sequels" && (
                <RrUserListFilters
                  ...
                  searchPlaceholder={t("aquila.searchManga")}
                />
              )}
            </div>

            {activeList === "Similar" || activeList === "Sequels" ? (
              <RrUserListSequelsTab username={username} mediaType="manga" />
            ) : (
              <>
                ...
                <InfiniteScroll
                  onLoadMore={() => {
                    fetchMangaList(nextCursor, false);
                  }}
                  hasMore={hasMore}
                  isLoading={loading}
                />
