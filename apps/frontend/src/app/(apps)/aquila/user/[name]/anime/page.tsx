...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description = formatUserListDescription("anime", counts, displayName);
...
export default async function Page({ params }: PageProps) {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/list/anime/user/${name}?limit=30&status=All&sort=last_updated`,
    ...
    console.error("Failed to fetch initial anime list data on server", e);
  }

  return <UserAnimePage initialData={initialData} />;
}
