...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description = formatUserListDescription("book", counts, displayName);
...
export default async function Page({ params }: PageProps) {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/list/book/user/${name}?limit=30&status=All&sort=last_updated`,
