...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description = formatUserListDescription("tv", counts, displayName);
...
export default async function Page({ params }: PageProps) {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/list/tv/user/${name}?limit=30&status=All&sort=last_updated`,
