...
export default function UserTvPage({ initialData }: { initialData?: any }) {
  ...
  useEffect(() => {
    ...
    setHasMore(true);
    if (activeList === "All") {
      fetchTvList(undefined, true, TV_PRIORITY_STATUSES[0], controller.signal);
    } else {
      fetchTvList(undefined, true, undefined, controller.signal);
    }
    return () => {
      controller.abort();
    };
            ...
            {activeList === "Similar" || activeList === "Sequels" ? (
              ...
              <>
                <header className="flex items-center justify-between mt-4">
                  ...
                  >
                    {getListNameTranslation(activeList)} {t("aquila.tv")} (
                ...
                <RrMediaListDisplay
                  ...
                  onRefresh={() => {
                    if (activeList === "All") {
                      setPriorityIdx(0);
                      setPriorityCursor(undefined);
                      fetchTvList(undefined, true, TV_PRIORITY_STATUSES[0]);
                    } else {
                      setNextCursor(undefined);
                      fetchTvList(undefined, true);
                    }
                  }}
                />
