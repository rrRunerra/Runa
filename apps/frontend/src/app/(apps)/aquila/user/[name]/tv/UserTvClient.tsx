...
export default function UserTvPage() {
  ...
  const fetchTvList = (currentOffset = 0, isReset = false, statusOverride?: string) => {
    if (!username) return;
          ...
          <>
            ...
            <header className="flex items-center justify-between mt-4">
              <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground/75">
                {activeList} TV ({counts?.[activeList.toLowerCase().replace(/\s+/g, "_")] ?? tvList.length})
              </h3>
            ...
            <MediaListDisplay
              ...
              baseUrl="/aquila/tv"
              isOwner={isOwner}
              onRefresh={() => fetchTvList(0, true, activeList === "All" ? TV_PRIORITY_STATUSES[0] : undefined)}
            />

            <InfiniteScroll
              onLoadMore={() => {
                if (activeList === "All") {
                  fetchTvList(priorityOff, false, TV_PRIORITY_STATUSES[priorityIdx]);
                } else {
