...
export default function UserGamesPage({ initialData }: { initialData?: any }) {
  ...
  useEffect(() => {
    ...
    setHasMore(true);
    if (activeList === "All") {
      fetchGameList(undefined, true, GAMES_PRIORITY_STATUSES[0], controller.signal);
    } else {
      fetchGameList(undefined, true, undefined, controller.signal);
    }
    return () => {
      controller.abort();
    };
  ...
  return (
    ...
    >
      ...
      >
        <RrUserListHeader userData={userData || null} listTitle="Games List" entries={gameList} />
            ...
            <div className="flex flex-col gap-4 w-full bg-card/20 backdrop-blur-xl border border-border/40 p-4 rounded-2xl shadow-xl">
              ...
              {activeList !== "Sequels" && (
                <RrUserListFilters
                  ...
                  searchPlaceholder={t("aquila.searchGames")}
                />
              )}
            </div>

            {activeList === "Sequels" ? (
              <RrUserListSequelsTab username={username} mediaType="game" />
            ) : (
              <>
                ...
                <RrMediaListDisplay
                  ...
                  onRefresh={() => {
                    if (activeList === "All") {
                      setPriorityIdx(0);
                      setPriorityCursor(undefined);
                      fetchGameList(undefined, true, GAMES_PRIORITY_STATUSES[0]);
                    } else {
                      setNextCursor(undefined);
                      fetchGameList(undefined, true);
                    }
                  }}
                />
                  ...
                  onLoadMore={() => {
                    if (activeList === "All") {
                      fetchGameList(
                        priorityCursor,
                        false,
                        GAMES_PRIORITY_STATUSES[priorityIdx],
                      );
                    } else {
                      fetchGameList(nextCursor, false);
                    }
                  }}
                  hasMore={hasMore}
                  isLoading={loading}
