...
const SORT_OPTIONS = [
  ...
  { label: "Chapter Count", value: "episode_count" },
  { label: "Last Updated", value: "last_updated" },
  { label: "Last Added", value: "last_added" },
];

export default function UserBooksPage({ initialData }: { initialData?: any }) {
  ...
  useEffect(() => {
    ...
    setHasMore(true);
    fetchBookList(undefined, true, controller.signal);
    return () => {
      controller.abort();
    };
  }, [
            ...
            <div className="flex flex-col gap-4 w-full bg-card/20 backdrop-blur-xl border border-border/40 p-4 rounded-2xl shadow-xl">
              ...
              {activeList !== "Similar" && activeList !== "Sequels" && (
                <RrUserListFilters
                  ...
                  searchPlaceholder={t("aquila.searchBooks")}
                />
              )}
            </div>

            {activeList === "Similar" || activeList === "Sequels" ? (
              <RrUserListSequelsTab username={username} mediaType="book" />
            ) : (
              <>
                <header className="flex items-center justify-between mt-4">
                  ...
                  >
                    {getListNameTranslation(activeList)} {t("aquila.books")} (
