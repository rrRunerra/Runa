...
const BOOKS_PRIORITY_STATUSES = ["Reading", "Completed", "Dropped", "Planning", "On Hold"];
...
export default function UserBooksPage() {
  ...
  const [bookList, setBookList] = useState<MediaEntry[]>([]);
  ...
  const fetchBookList = (currentOffset = 0, isReset = false, statusOverride?: string) => {
    if (!username) return;
    ...
    fetch(
      ...
      .catch((err) => console.error("Failed to fetch books list", err))
      .finally(() => {
        setLoading(false);
  ...
  useEffect(() => {
    ...
    setHasMore(true);
    if (activeList === "All") {
      fetchBookList(0, true, BOOKS_PRIORITY_STATUSES[0]);
    } else {
  ...
  useEffect(() => {
    if (!userData) return;
    document.title = `Aquila > User > ${userData?.displayName || userData?.username} > Books List`;
  }, [userData]);
          ...
          <>
            ...
            <header className="flex items-center justify-between mt-4">
              <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground/75">
                {activeList} Books ({counts?.[activeList.toLowerCase().replace(/\s+/g, "_")] ?? bookList.length})
              </h3>
            ...
            <MediaListDisplay
              ...
              isOwner={isOwner}
              onRefresh={() => fetchBookList(0, true, activeList === "All" ? BOOKS_PRIORITY_STATUSES[0] : undefined)}
            />

            <InfiniteScroll
              onLoadMore={() => {
                if (activeList === "All") {
                  fetchBookList(priorityOff, false, BOOKS_PRIORITY_STATUSES[priorityIdx]);
                } else {
