...
export default function UserBooksPage() {
  ...
  const fetchBookList = () => {
    if (username) {
      ...
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/list/book/user/${username}`, {
        headers,
      })
        .then(async (res) => {
          if (res.status === 403) {
  ...
  }, [search, activeList, bookList]);  const [isSheetOpen, setIsSheetOpen] = useState(false);
