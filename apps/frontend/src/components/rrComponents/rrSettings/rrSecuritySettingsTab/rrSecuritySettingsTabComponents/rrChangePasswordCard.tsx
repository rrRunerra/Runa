...
export interface RrChangePasswordCardProps {
  ...
  /** Async callback fired to change the user's password */
  onChangePassword: (current: string, newPass: string) => Promise<void>;
}

/**
 * Card component for updating the user's password.
 * Includes password criteria validation, visibility toggles, and matching checks.
 */
    ...
    <div className="flex flex-col gap-4 text-left h-full">
      <Card className="h-full flex flex-col justify-between">
        <div>
          ...
          <CardContent className="flex flex-col gap-5 pt-2">
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  aria-label={
                    showCurrentPassword
                      ? t("securitySettings.hidePassword")
                      : t("securitySettings.showPassword")
                  }
                >
                  {showCurrentPassword ? (
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="sec-new-password">
                ...
                <div className="mt-1.5 p-3 rounded-xl bg-muted/35 border border-border/50 animate-in fade-in slide-in-from-top-1 duration-150">
                  ...
                  <ul className="grid grid-cols-1 gap-1.5">
                    {[
                      {
                        key: "length",
                        label: t("securitySettings.critMinLength"),
                      },
                      {
                        key: "maxLength",
                        label: t("securitySettings.critMaxLength"),
                      },
                      {
                        key: "uppercase",
                        label: t("securitySettings.critUppercase"),
                      },
                      {
                        key: "number",
                        label: t("securitySettings.critNumbers"),
                      },
                      {
                        key: "special",
                        label: t("securitySettings.critSpecial"),
                      },
                    ].map((item) => (
                      <li
                        key={item.key}
      ...
      {showSaveBar && (
        <div className="flex justify-end gap-3 p-4 rounded-xl border border-border bg-muted/10 animate-in fade-in duration-200">
          ...
          >
            {isSubmitting
              ? t("securitySettings.updating")
              : t("securitySettings.updatePasswordBtn")}
          </Button>
        </div>
      )}
    </div>
