...
} from "./rrMailSettingsTabComponents/rrMailProviderPresets";
import { RrMailLinkWizardStep1 } from "./rrMailSettingsTabComponents/rrMailLinkWizardStep1";
import {
  RrMailLinkWizardStep2,
  type ConnectionTestState,
} from "./rrMailSettingsTabComponents/rrMailLinkWizardStep2";
...
/**
 * Main component managing linked email accounts, IMAP/SMTP configurations, and email setup wizard.
 */
export function RrMailSettingsTab({
  ...
  const [selectedProvider, setSelectedProvider] =
    useState<RrMailProviderPreset>(
      MAIL_PROVIDER_PRESETS.find((p) => p.id === "purelymail") ||
        MAIL_PROVIDER_PRESETS[0],
    );
  ...
  const { data: emailAccountsData, mutate: refetchEmailAccounts } = useSWR<
    any[]
  >(
    session?.accessToken
  ...
  useEffect(() => {
    if (emailAccountsData) {
      setEmailAccounts(
        Array.isArray(emailAccountsData) ? emailAccountsData : [],
      );
    }
  }, [emailAccountsData]);
  ...
  const resetForm = () => {
    ...
    setCurrentStep(1);
    const customPreset =
      MAIL_PROVIDER_PRESETS.find((p) => p.id === "custom") ||
      MAIL_PROVIDER_PRESETS[0];
  ...
  const handleAutodetect = async (): Promise<void> => {
    ...
    try {
      const config = await apiMutate(
        `${process.env.NEXT_PUBLIC_API_URL}/emails/autoconfig/${domain}`,
        "GET",
      );
  ...
  const handleTestConnection = async (): Promise<boolean> => {
    ...
    try {
      const res = await apiMutate(
        `${process.env.NEXT_PUBLIC_API_URL}/emails/test-connection`,
        "POST",
        payload,
      );
  ...
  const handleSaveEmailAccount = async (): Promise<void> => {
    ...
    try {
      if (editingAccount) {
        await apiMutate(
          `${process.env.NEXT_PUBLIC_API_URL}/emails/${editingAccount.id}`,
          "PUT",
          payload,
        );
      } else {
      ...
      toast.success(
        editingAccount
          ? t("mailSettings.toastUpdated")
          : t("mailSettings.toastAdded"),
      );
          ...
          <DialogHeader className="pb-3 border-b border-border/40 text-left pr-8">
            <div className="flex items-center justify-between">
              <DialogTitle className="text-base font-bold">
                {editingAccount
                  ? t("mailSettings.editTitle")
                  : t("mailSettings.linkTitle")}
              </DialogTitle>
