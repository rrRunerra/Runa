...
export function RrMediaSection({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 sm:gap-3 pb-2 sm:pb-3 group/header">
        <div
          {...(dragHandleProps || {})}
          className={cn(
            "p-1.5 sm:p-2.5 bg-primary/10 text-primary border border-primary/20 rounded-lg sm:rounded-xl select-none flex items-center gap-1 sm:gap-1.5 shadow-xs",
            dragHandleProps &&
        ...
        >
          {dragHandleProps && (
            <GripVertical className="size-3.5 sm:size-4 opacity-50 group-hover/header:opacity-100 transition-opacity duration-200" />
          )}
          {icon}
        </div>
        ...
        <button onClick={toggleCollapse} className="cursor-pointer select-none">
          <Badge
            ...
            className={cn(
              "ml-1 sm:ml-2 transition-all duration-300 flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs font-semibold py-0.5 px-2 sm:px-2.5 rounded-full select-none cursor-pointer border",
              isCollapsed
      ...
      <AnimatePresence initial={false}>
        {!isCollapsed && (
          ...
          >
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-2 sm:gap-3.5 md:gap-4 pb-2">
