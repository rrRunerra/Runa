...
const GENRE_OPTIONS: Record<string, string[]> = {
  ...
  tv: [
    ...
    "Drama", "Family", "Fantasy", "History", "Horror", "Mystery", "Reality",
    "Romance", "Sci-Fi", "Talk Show", "Thriller", "War", "Western",
  ],
  movie: [
    "Action", "Adventure", "Animation", "Comedy", "Crime", "Documentary",
...
const STATUS_OPTIONS: Record<string, string[]> = {
  ...
  movie: ["Released", "In Production", "Post Production", "Planned", "Canceled"],
...
export function RrMediaSubmissionModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({
        titleEnglish: initialData?.titleEnglish || initialData?.titleString || initialData?.title?.english || "",
        titleRomaji: initialData?.titleRomaji || initialData?.title?.romaji || "",
        ...
        bannerImage: initialData?.bannerImage || initialData?.backgroundImage || "",
        ...
        duration: initialData?.duration || initialData?.runtime || initialData?.averageRuntime || "",
        ...
        startDateYear: initialData?.startDateYear || initialData?.releasedYear || initialData?.publishedYear || "",
        startDateMonth: initialData?.startDateMonth || initialData?.releasedMonth || initialData?.publishedMonth || "",
        startDateDay: initialData?.startDateDay || initialData?.releasedDay || initialData?.publishedDay || "",
        ...
        countryOfOrigin: initialData?.countryOfOrigin || initialData?.originalCountry || "JP",
        originalLanguage: initialData?.originalLanguage || "ja",
        contentRating: initialData?.contentRating || initialData?.esrbRating || "",
        publisher: initialData?.publisher || "",
        authors: Array.isArray(initialData?.authors) ? initialData.authors.join(", ") : "",
        artists: Array.isArray(initialData?.artists) ? initialData.artists.join(", ") : "",
        platforms: Array.isArray(initialData?.platforms) ? initialData.platforms.join(", ") : "",
        developers: Array.isArray(initialData?.developers) ? initialData.developers.join(", ") : "",
        publishers: Array.isArray(initialData?.publishers) ? initialData.publishers.join(", ") : "",
      ...
      if (Array.isArray(initialData?.characters)) {
        charList = initialData.characters.map((c: any) => ({
          ...
          name: c.name || [c.nameFirst || c.character?.nameFirst, c.nameLast || c.character?.nameLast].filter(Boolean).join(" ") || c.nameNative || c.character?.nameNative || "Character",
          role: c.role || "MAIN",
        ...
        charList = initialData.characters.edges.map((e: any) => ({
          ...
          name: e.node?.name?.userPreferred || [e.node?.name?.first, e.node?.name?.last].filter(Boolean).join(" ") || "Character",
          role: e.role || "MAIN",
        ...
        charList = initialData.animeCharacters.map((ac: any) => ({
          ...
          name: [ac.character?.nameFirst, ac.character?.nameLast].filter(Boolean).join(" ") || ac.character?.nameNative || "Character",
          role: ac.role || "MAIN",
        ...
        charList = initialData.mangaCharacters.map((mc: any) => ({
          ...
          name: [mc.character?.nameFirst, mc.character?.nameLast].filter(Boolean).join(" ") || mc.character?.nameNative || "Character",
          role: mc.role || "MAIN",
      ...
      const parseRelationTitle = (item: any): string => {
        ...
        if (typeof item.title === "object" && item.title) {
          return item.title.english || item.title.romaji || item.title.native || item.title.userPreferred || "Related Media";
        }
        return item.titleEnglish || item.titleRomaji || item.titleNative || item.titleString || item.node?.title?.userPreferred || item.node?.title?.english || item.node?.title?.romaji || "Related Media";
      };
      ...
      if (Array.isArray(initialData?.relations)) {
        relList = initialData.relations.map((r: any) => ({
          relatedMediaId: r.relatedMediaId || r.id || r.relatedAnimeId || r.relatedMangaId,
          title: parseRelationTitle(r),
          relationType: r.relationType || "SEQUEL",
          image: r.image || r.coverImage || r.coverImageLarge || r.node?.coverImage?.large,
        }));
        ...
        relList = initialData.animeRelations.map((ar: any) => ({
          relatedMediaId: ar.relatedAnimeId || ar.relatedMangaId || ar.relatedAnime?.id || ar.relatedManga?.id,
          title: parseRelationTitle(ar.relatedAnime || ar.relatedManga),
          relationType: ar.relationType || "SEQUEL",
          image: ar.relatedAnime?.coverImageLarge || ar.relatedManga?.coverImageLarge,
        }));
      } else if (Array.isArray(initialData?.mangaRelations)) {
        relList = initialData.mangaRelations.map((mr: any) => ({
          relatedMediaId: mr.relatedMangaId || mr.relatedAnimeId || mr.relatedManga?.id || mr.relatedAnime?.id,
          title: parseRelationTitle(mr.relatedManga || mr.relatedAnime),
          relationType: mr.relationType || "SEQUEL",
          image: mr.relatedManga?.coverImageLarge || mr.relatedAnime?.coverImageLarge,
        }));
      }
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.titleEnglish && !formData.titleRomaji && !formData.titleNative) {
    ...
    const payloadData = {
      ...
      authors: formData.authors ? formData.authors.split(",").map((s: string) => s.trim()).filter(Boolean) : [],
      artists: formData.artists ? formData.artists.split(",").map((s: string) => s.trim()).filter(Boolean) : [],
      platforms: formData.platforms ? formData.platforms.split(",").map((s: string) => s.trim()).filter(Boolean) : [],
      developers: formData.developers ? formData.developers.split(",").map((s: string) => s.trim()).filter(Boolean) : [],
      publishers: formData.publishers ? formData.publishers.split(",").map((s: string) => s.trim()).filter(Boolean) : [],
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="!w-[90vw] !max-w-[90vw] sm:!max-w-[90vw] !h-[90vh] !max-h-[90vh] flex flex-col p-0 gap-0 overflow-hidden bg-background/95 backdrop-blur-2xl border-border/60 rounded-2xl shadow-2xl">
        <DialogHeader className="p-6 border-b border-border/40 pb-4">
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-xl font-bold flex items-center gap-2">
                <Sparkles className="size-5 text-primary" />
                {actionType === "EDIT" ? "Edit Media Metadata" : "Add New Media Entry"}
              </DialogTitle>
            ...
            {actionType === "CREATE" && (
              ...
              >
                <SelectTrigger className="w-[140px] h-9 text-xs font-semibold capitalize">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
          <div className="px-6 pt-3 border-b border-border/40 bg-muted/20">
            <TabsList className="flex items-center justify-start gap-2 h-10 bg-muted/50 p-1 rounded-xl w-full overflow-x-auto">
              <TabsTrigger value="general" className="px-4 text-xs rounded-lg font-medium whitespace-nowrap">General</TabsTrigger>
              <TabsTrigger value="images" className="px-4 text-xs rounded-lg font-medium whitespace-nowrap">Images & Links</TabsTrigger>
              <TabsTrigger value="genres" className="px-4 text-xs rounded-lg font-medium whitespace-nowrap">Genres & Tags</TabsTrigger>
              <TabsTrigger value="characters" className="px-4 text-xs rounded-lg font-medium whitespace-nowrap">Characters & Staff</TabsTrigger>
              <TabsTrigger value="relations" className="px-4 text-xs rounded-lg font-medium whitespace-nowrap">Relations</TabsTrigger>
            </TabsList>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <TabsContent value="general" className="m-0 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1.5 md:col-span-1">
                  <Label className="text-xs font-semibold">Title (Romaji)</Label>
                  <Input
                    placeholder="e.g. Sousou no Frieren"
                    value={formData.titleRomaji || ""}
                    onChange={(e) => handleChange("titleRomaji", e.target.value)}
                  />
                </div>
                <div className="space-y-1.5 md:col-span-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleChange("titleNative", e.target.value)}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {FORMAT_OPTIONS[selectedMediaType] && (
                  <div className="space-y-1.5">
                    ...
                    <Select
                      value={formData.format || FORMAT_OPTIONS[selectedMediaType][0]}
                ...
                {STATUS_OPTIONS[selectedMediaType] && (
                  <div className="space-y-1.5">
                    ...
                    <Select
                      value={formData.status || STATUS_OPTIONS[selectedMediaType][0]}
                ...
                {(selectedMediaType === "anime" || selectedMediaType === "manga" || selectedMediaType === "book") && (
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">
                      {selectedMediaType === "anime" ? "Episodes" : selectedMediaType === "manga" ? "Chapters" : "Pages"}
                    </Label>
                    <Input
                      type="number"
                      placeholder="e.g. 28"
                      value={selectedMediaType === "anime" ? formData.episodes || "" : selectedMediaType === "manga" ? formData.chapters || "" : formData.pageCount || ""}
                      onChange={(e) =>
                        handleChange(
                          selectedMediaType === "anime" ? "episodes" : selectedMediaType === "manga" ? "chapters" : "pageCount",
                          e.target.value
                        )
                      }
                    />
                ...
                {(selectedMediaType === "anime" || selectedMediaType === "tv" || selectedMediaType === "movie") && (
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">Duration / Runtime (mins)</Label>
                    <Input
                      type="number"
                      placeholder="e.g. 24"
                ...
                {(selectedMediaType === "anime" || selectedMediaType === "manga") && (
                  <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => handleChange("startDateYear", e.target.value)}
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => handleChange("startDateMonth", e.target.value)}
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => handleChange("startDateDay", e.target.value)}
                    />
                  </div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleChange("endDateYear", e.target.value)}
                      />
                    </div>
                    <div className="space-y-1">
                      <Label className="text-[11px]">Month</Label>
                      <Input
                        ...
                        onChange={(e) => handleChange("endDateMonth", e.target.value)}
                      />
                    </div>
                    <div className="space-y-1">
                      <Label className="text-[11px]">Day</Label>
                      <Input
                        ...
                        onChange={(e) => handleChange("endDateDay", e.target.value)}
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2 border-t border-border/30">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    value={formData.countryOfOrigin || formData.originalCountry || ""}
                ...
                {selectedMediaType === "book" && (
                  <>
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold">Authors (comma separated)</Label>
                      <Input
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold">Publishers</Label>
                      <Input
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold">ISBN-10</Label>
                      <Input
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold">ISBN-13</Label>
                      <Input
                ...
                {selectedMediaType === "game" && (
                  <>
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold">Developers</Label>
                      <Input
                        placeholder="e.g. FromSoftware"
                        value={formData.developers || ""}
                        onChange={(e) => handleChange("developers", e.target.value)}
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold">Publishers</Label>
            ...
            <TabsContent value="images" className="m-0 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <div className="relative">
                    <Button variant="outline" size="sm" className="gap-1.5 text-xs h-9" disabled={isUploadingCover}>
                      {isUploadingCover ? <Spinner className="size-3" /> : <Upload className="size-3.5" />}
                      Upload File
                    </Button>
                    <input
                      type="file"
                ...
                {formData.coverImage && (
                  <div className="mt-2 w-24 h-36 rounded-xl overflow-hidden border border-border/40">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <div className="relative">
                    <Button variant="outline" size="sm" className="gap-1.5 text-xs h-9" disabled={isUploadingBanner}>
                      {isUploadingBanner ? <Spinner className="size-3" /> : <Upload className="size-3.5" />}
                      Upload File
                    </Button>
                    <input
                      type="file"
                ...
                {formData.bannerImage && (
                  <div className="mt-2 w-full h-24 rounded-xl overflow-hidden border border-border/40">
            ...
            <TabsContent value="genres" className="m-0 space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5 p-3 rounded-xl border border-border/40 bg-muted/20">
                  {(GENRE_OPTIONS[selectedMediaType] || GENRE_OPTIONS.anime).map((genre) => {
              ...
              {/* Synonyms */}
              <div className="space-y-2">
                <Label className="text-xs font-semibold">Alternative Titles / Synonyms</Label>
                <div className="flex gap-2">
                  <Input
                  ...
                  />
                  <Button type="button" size="sm" variant="secondary" onClick={handleAddSynonym}>
                    <Plus className="size-4" />
                  </Button>
                </div>
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {synonymsList.map((syn) => (
                    <Badge key={syn} variant="secondary" className="gap-1 text-xs">
                      {syn}
                      <Trash2
            ...
            <TabsContent value="characters" className="m-0 space-y-4">
              <div className="space-y-2">
                <Label className="text-xs font-semibold">Attach Character from Database</Label>
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), handleCharacterSearchSubmit())}
                      className="pl-9"
                    />
                  </div>
                  <Button type="button" size="sm" variant="secondary" onClick={handleCharacterSearchSubmit} className="gap-1.5 text-xs">
                ...
                {characterResults.length > 0 && (
                  <div className="p-2 border border-border/40 rounded-xl bg-card max-h-52 overflow-y-auto space-y-1.5">
                    {characterResults.map((char) => {
                      const fullName = [char.nameFirst, char.nameMiddle, char.nameLast].filter(Boolean).join(" ") || char.nameNative || "Character";
                      return (
                        <div
                          key={char.id}
                          className="flex items-center justify-between p-2 rounded-lg hover:bg-muted/50 text-xs cursor-pointer gap-3"
                          onClick={() => {
                            if (!selectedCharacters.some((c) => c.characterId === char.id)) {
                              setSelectedCharacters((prev) => [
                                ...prev,
                                { characterId: char.id, name: fullName, role: "MAIN", image: char.image },
                              ]);
                            }
                            setCharacterSearch("");
                            setCharacterResults([]);
                        ...
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            {char.image ? (
                              <img src={char.image} alt={fullName} className="w-8 h-8 rounded-full object-cover shrink-0" />
                            ) : (
                              <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-[10px] shrink-0 font-bold">
                                {fullName.charAt(0)}
                              </div>
                            )}
                            <div className="min-w-0">
                              <p className="font-semibold truncate">{fullName}</p>
                              {char.nameNative && <p className="text-[10px] text-muted-foreground truncate">{char.nameNative}</p>}
                            </div>
                          </div>
              ...
              {/* Selected Characters */}
              <div className="space-y-2">
                <Label className="text-xs font-semibold">Selected Characters ({selectedCharacters.length})</Label>
                <div className="space-y-2">
                  {selectedCharacters.map((char, index) => (
                    <div key={index} className="flex items-center justify-between p-2 rounded-xl border border-border/40 bg-muted/20 text-xs gap-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        {char.image ? (
                          <img src={char.image} alt={char.name} className="w-8 h-8 rounded-full object-cover shrink-0" />
                        ) : (
                          <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-[10px] shrink-0 font-bold">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <Button
                          ...
                          onClick={() => setSelectedCharacters((prev) => prev.filter((_, i) => i !== index))}
                        >
            ...
            <TabsContent value="relations" className="m-0 space-y-4">
              <div className="space-y-2">
                <Label className="text-xs font-semibold">Search Related Media</Label>
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" />
                    <Input
                      ...
                      onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), handleRelationSearchSubmit())}
                      className="pl-9"
                    />
                  </div>
                  <Button type="button" size="sm" variant="secondary" onClick={handleRelationSearchSubmit} className="gap-1.5 text-xs">
                    <Search className="size-3.5" />
                    Search
                  </Button>
                </div>
                ...
                {relationResults.length > 0 && (
                  <div className="p-2 border border-border/40 rounded-xl bg-card max-h-52 overflow-y-auto space-y-1.5">
                    {relationResults.map((rel) => {
                      const title = rel.titleEnglish || rel.titleRomaji || rel.titleString || "Media";
                      ...
                      return (
                        <div
                          ...
                          className="flex items-center justify-between p-2 rounded-lg hover:bg-muted/50 text-xs cursor-pointer gap-3"
                          onClick={() => {
                            if (!selectedRelations.some((r) => r.relatedMediaId === rel.id)) {
                              setSelectedRelations((prev) => [
                                ...prev,
                                { relatedMediaId: rel.id, title, relationType: "SEQUEL", image: img },
                              ]);
                            }
                            setRelationSearch("");
                            setRelationResults([]);
                        ...
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            {img ? (
                              <img src={img} alt={title} className="w-7 h-10 rounded-md object-cover shrink-0" />
                            ) : (
              ...
              {/* Selected Relations */}
              <div className="space-y-2">
                <Label className="text-xs font-semibold">Selected Relations ({selectedRelations.length})</Label>
                <div className="space-y-2">
                  {selectedRelations.map((rel, index) => (
                    <div key={index} className="flex items-center justify-between p-2 rounded-xl border border-border/40 bg-muted/20 text-xs gap-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        {rel.image ? (
                          <img src={rel.image} alt={rel.title} className="w-7 h-10 rounded-md object-cover shrink-0" />
                        ) : (
                          <div className="w-7 h-10 rounded-md bg-muted flex items-center justify-center text-[10px] shrink-0 font-bold">
                            M
                          </div>
                        )}
                        <span className="font-medium truncate max-w-[200px]">{rel.title}</span>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <Select
                        ...
                        <Button
                          ...
                          variant="ghost"
                          className="h-7 w-7 text-destructive hover:bg-destructive/10"
                          onClick={() => setSelectedRelations((prev) => prev.filter((_, i) => i !== index))}
                        >
                          <Trash2 className="size-3.5" />
                        </Button>
                      </div>
        ...
        <DialogFooter className="p-4 border-t border-border/40 flex items-center justify-between bg-muted/20">
          <Button variant="ghost" size="sm" onClick={onClose} disabled={isSubmitting}>
            Cancel
          </Button>
          <Button size="sm" onClick={handleSubmit} disabled={isSubmitting} className="gap-2 font-semibold">
