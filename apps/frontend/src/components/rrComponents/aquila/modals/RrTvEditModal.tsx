...
import { Sparkles, Info, Calendar, ImageIcon, Tag, Database, Users, Tv, Lock, Unlock, Calculator, Radio } from "lucide-react";
...
import { RrSubmissionEpisodesTab } from "../submission/rrSubmissionEpisodesTab";
import { RrSubmissionInternalMetricsTab } from "../submission/rrSubmissionInternalMetricsTab";

export interface RrTvEditModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const FORMATS = ["Scripted", "Animation", "Reality", "Documentary", "Talk Show", "News"];
const STATUSES = ["RETURNING_SERIES", "ENDED", "CANCELED", "IN_PRODUCTION", "UPCOMING", "UNKNOWN"];
const SOURCES = ["Original", "Novel", "Book", "Comic", "Game", "True Story"];

export function RrTvEditModal({
  isOpen,
  onClose,
  actionType = "EDIT",
  mediaId,
  initialData = {},
  onSuccess,
}: RrTvEditModalProps): React.JSX.Element {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({
        ...
        format: initialData?.format || initialData?.showType || "Scripted",
        showType: initialData?.showType || initialData?.format || "Scripted",
        status: initialData?.status || "RETURNING_SERIES",
        ...
        seasonCount: initialData?.seasonCount || "",
        duration: initialData?.duration || initialData?.averageRuntime || initialData?.runtime || "",
        averageRuntime: initialData?.averageRuntime || initialData?.duration || "",
        broadcastTime: initialData?.broadcastTime || "",
        broadcastDays: Array.isArray(initialData?.broadcastDays) ? initialData.broadcastDays.join(", ") : initialData?.broadcastDays || "",
        networks: Array.isArray(initialData?.networks) ? initialData.networks.join(", ") : initialData?.networks || "",
        startDateYear: initialData?.startDateYear || initialData?.firstAiredYear || "",
        startDateMonth: initialData?.startDateMonth || initialData?.firstAiredMonth || "",
        startDateDay: initialData?.startDateDay || initialData?.firstAiredDay || "",
        ...
        tvmazeId: initialData?.tvmazeId || "",
        tvrageId: initialData?.tvrageId || "",
      });
      ...
      if (Array.isArray(initialData?.episodes)) setEpisodesList(initialData.episodes);
      else setEpisodesList([]);
  ...
  const handleSubmit = async () => {
    ...
    const payloadData = {
      ...
      networks: typeof formData.networks === "string" ? formData.networks.split(",").map((s: string) => s.trim()).filter(Boolean) : formData.networks,
      broadcastDays: typeof formData.broadcastDays === "string" ? formData.broadcastDays.split(",").map((s: string) => s.trim()).filter(Boolean) : formData.broadcastDays,
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorJson.message || "Failed to submit TV show data");
      }

      toast.success("TV show submission sent for administrator review!");
          ...
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-xl font-bold tracking-tight flex items-center gap-2 text-foreground">
                ...
                {actionType === "EDIT" ? "Edit TV Show Metadata" : "Add New TV Show Entry"}
              </DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground mt-1">
                Edit seasons, episodes, broadcast networks, air times, and lock state.
              </DialogDescription>
            </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
          <div className="px-6 pt-3 pb-3 border-b border-border/50 bg-muted/15">
            <TabsList className="flex items-center justify-start gap-1 h-11 bg-muted/60 p-1 rounded-xl w-full overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden scrollbar-none border border-border/50 shadow-2xs">
              ...
              <TabsTrigger value="release" className="gap-1.5 px-3 text-xs rounded-lg font-bold transition-all data-[state=active]:bg-background data-[state=active]:text-primary whitespace-nowrap">
                <Calendar className="size-3.5" /> Seasons & Air Dates
              </TabsTrigger>
              ...
              <TabsTrigger value="broadcast" className="gap-1.5 px-3 text-xs rounded-lg font-bold transition-all data-[state=active]:bg-background data-[state=active]:text-primary whitespace-nowrap">
                <Radio className="size-3.5" /> Networks & Broadcast
              </TabsTrigger>
              ...
              <TabsTrigger value="people" className="gap-1.5 px-3 text-xs rounded-lg font-bold transition-all data-[state=active]:bg-background data-[state=active]:text-primary whitespace-nowrap">
                <Users className="size-3.5" /> Cast, Staff & Studios
              </TabsTrigger>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            <TabsContent value="release">
              <RrSubmissionReleaseTab mediaType="tv" formData={formData} onChange={handleChange} />
            </TabsContent>

            <TabsContent value="episodes">
              <RrSubmissionEpisodesTab
                mediaType="tv"
                formData={formData}
                onChange={handleChange}
            ...
            <TabsContent value="broadcast">
              <div className="space-y-4 bg-card/40 border border-border/60 rounded-2xl p-5 shadow-xs">
                <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                  <Radio className="size-4 text-primary" /> Broadcast Networks & Air Time Details
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-muted-foreground">Broadcast Networks (comma separated)</Label>
                    <Input
                      placeholder="e.g. HBO, Netflix, AMC, BBC One"
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-muted-foreground">Broadcast Days (comma separated)</Label>
                    <Input
                      placeholder="e.g. Sunday, Wednesday"
                      value={formData.broadcastDays || ""}
                      onChange={(e) => handleChange("broadcastDays", e.target.value)}
                      className="bg-background/80 border-border/70 rounded-xl h-10 text-xs font-medium"
                    />
                  </div>

                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-muted-foreground">Broadcast Time</Label>
                    <Input
                      placeholder="e.g. 21:00 EST"
                      value={formData.broadcastTime || ""}
                      onChange={(e) => handleChange("broadcastTime", e.target.value)}
                      className="bg-background/80 border-border/70 rounded-xl h-10 text-xs font-medium"
                    />
                  </div>

                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-muted-foreground">Show Type / Format</Label>
                    <Input
                      placeholder="e.g. Scripted, Reality, Animation"
                      value={formData.showType || ""}
                      onChange={(e) => handleChange("showType", e.target.value)}
                      className="bg-background/80 border-border/70 rounded-xl h-10 text-xs font-medium"
                    />
                  </div>
                </div>
            ...
            <TabsContent value="people">
              <RrSubmissionPeopleTab
                ...
                onSearchRelations={async (q) => {
                  try {
                    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/aquila/submissions/search/relations?mediaType=tv&q=${encodeURIComponent(q)}`);
                    if (res.ok) return await res.json();
                  } catch {}
                  return [];
                }}
            ...
            <TabsContent value="external">
              <RrSubmissionExternalIdsTab mediaType="tv" formData={formData} onChange={handleChange} />
            </TabsContent>

            <TabsContent value="metrics">
              <RrSubmissionInternalMetricsTab mediaType="tv" initialData={initialData} />
            </TabsContent>
          </div>
          ...
          <DialogFooter className="p-4 border-t border-border/50 bg-muted/20 gap-3">
            ...
            <Button type="button" onClick={handleSubmit} disabled={isSubmitting} className="h-10 px-6 rounded-xl text-xs font-bold bg-primary text-primary-foreground hover:bg-primary/90">
              {isSubmitting ? "Submitting..." : "Submit TV Show Edits"}
            </Button>
          </DialogFooter>
        </Tabs>
      </DialogContent>
