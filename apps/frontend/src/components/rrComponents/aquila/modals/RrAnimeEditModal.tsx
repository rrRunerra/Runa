...
export interface RrAnimeEditModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function RrAnimeEditModal({
  ...
  initialData = {},
  onSuccess,
}: RrAnimeEditModalProps): React.JSX.Element {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({
        ...
        nextAiringEpisodeNumber: initialData?.nextAiringEpisodeNumber || "",
        nextAiringAt: initialData?.nextAiringAt || "",
        anilistId: initialData?.anilistId || "",
        malId: initialData?.malId || "",
        aniDBId: initialData?.aniDBId || "",
        tvDBId: initialData?.tvDBId || "",
      ...
      if (Array.isArray(initialData?.episodes)) {
        setEpisodesList(initialData.episodes);
      } else {
        setEpisodesList([]);
      }

      // Prefill Characters, Staff, Studios, Relations
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorJson.message || "Failed to submit anime data");
      }

      toast.success("Anime submission submitted for administrator review!");
