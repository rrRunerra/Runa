...
import { Sparkles, Info, Calendar, ImageIcon, Tag, Database, Users, Lock, Unlock, Calculator, Film } from "lucide-react";
...
export interface RrMovieEditModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const FORMATS = ["Theatrical", "Direct to Video", "TV Movie", "Short"];
const STATUSES = ["RELEASED", "IN_PRODUCTION", "POST_PRODUCTION", "RUMORED", "CANCELLED", "UNKNOWN"];
const SOURCES = ["Original", "Book", "Comic", "Play", "Game", "Real Life"];

export function RrMovieEditModal({
  isOpen,
  onClose,
  actionType = "EDIT",
  mediaId,
  initialData = {},
  onSuccess,
}: RrMovieEditModalProps): React.JSX.Element {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({
        ...
        format: initialData?.format || "Theatrical",
        status: initialData?.status || "RELEASED",
        source: initialData?.source || "Original",
        runtime: initialData?.runtime || initialData?.duration || "",
        duration: initialData?.duration || initialData?.runtime || "",
        ...
        homepage: initialData?.homepage || initialData?.website || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorJson.message || "Failed to submit movie data");
      }

      toast.success("Movie submission sent for administrator review!");
          ...
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-xl font-bold tracking-tight flex items-center gap-2 text-foreground">
                ...
                {actionType === "EDIT" ? "Edit Movie Metadata" : "Add New Movie Entry"}
              </DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground mt-1">
                Edit runtime, budget, revenue, cast, crew, and lock state.
              </DialogDescription>
            </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
          <div className="px-6 pt-3 pb-3 border-b border-border/50 bg-muted/15">
            <TabsList className="flex items-center justify-start gap-1 h-11 bg-muted/60 p-1 rounded-xl w-full overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden scrollbar-none border border-border/50 shadow-2xs">
              ...
              <TabsTrigger value="release" className="gap-1.5 px-3 text-xs rounded-lg font-bold transition-all data-[state=active]:bg-background data-[state=active]:text-primary whitespace-nowrap">
                <Calendar className="size-3.5" /> Runtime & Box Office
              </TabsTrigger>
              ...
              <TabsTrigger value="people" className="gap-1.5 px-3 text-xs rounded-lg font-bold transition-all data-[state=active]:bg-background data-[state=active]:text-primary whitespace-nowrap">
                <Users className="size-3.5" /> Cast, Crew & Studios
              </TabsTrigger>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            <TabsContent value="release">
              <RrSubmissionReleaseTab mediaType="movie" formData={formData} onChange={handleChange} />
            </TabsContent>
            ...
            <TabsContent value="people">
              <RrSubmissionPeopleTab
                ...
                onSearchRelations={async (q) => {
                  try {
                    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/aquila/submissions/search/relations?mediaType=movie&q=${encodeURIComponent(q)}`);
                    if (res.ok) return await res.json();
                  } catch {}
                  return [];
                }}
            ...
            <TabsContent value="external">
              <RrSubmissionExternalIdsTab mediaType="movie" formData={formData} onChange={handleChange} />
            </TabsContent>

            <TabsContent value="metrics">
              <RrSubmissionInternalMetricsTab mediaType="movie" initialData={initialData} />
            </TabsContent>
          </div>
          ...
          <DialogFooter className="p-4 border-t border-border/50 bg-muted/20 gap-3">
            ...
            <Button type="button" onClick={handleSubmit} disabled={isSubmitting} className="h-10 px-6 rounded-xl text-xs font-bold bg-primary text-primary-foreground hover:bg-primary/90">
              {isSubmitting ? "Submitting..." : "Submit Movie Edits"}
            </Button>
          </DialogFooter>
        </Tabs>
      </DialogContent>
