...
export interface RrMediaCompactRowProps {
  entry: RrMediaEntry;
  baseUrl: string;
  isOwner?: boolean;
  onRefresh?: () => void;
...
function RrMediaCompactRowComponent({
  entry,
  baseUrl,
  isOwner,
  onRefresh,
}: RrMediaCompactRowProps): React.JSX.Element {
    ...
    <>
      <div className="flex items-center justify-between w-full hover:bg-muted/30 rounded-xl pr-2 transition-all group/row">
        <Link
          ...
          prefetch={false}
          className="flex-1 flex cursor-pointer items-center justify-between gap-4 py-2 px-2 transition-colors min-w-0"
        >
          <span className="font-semibold text-xs text-foreground group-hover/row:text-primary transition-colors truncate">
            {entry.title}
          </span>
          <div className="flex items-center gap-4 md:gap-6 text-xs text-muted-foreground w-[100px] sm:w-[180px] justify-end shrink-0">
            ...
            <span className="w-16 text-right hidden sm:block capitalize text-[10px] font-semibold text-muted-foreground/60">
              {entry.type}
            </span>
          </div>
        </Link>
...
export const RrMediaCompactRow = memo(
  RrMediaCompactRowComponent,
  (prevProps, nextProps) => {
    return (
