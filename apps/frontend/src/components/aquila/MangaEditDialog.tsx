...
export function MangaEditDialog({
      ...
      if (session.status === "authenticated" && session.data) {
        try {
          const favRes = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/favorites/status/manga/${initialMedia.id}`,
            {
              headers: {
            ...
            if (data) {
              ...
              for (const key of Object.keys(rawConnections)) {
                ...
                if (conn && typeof conn === 'object') {
                  let connChapters: string | undefined = undefined;
                  if (conn.chaptersOffset !== undefined) {
                    connChapters = (Number(data.chapters || 0) + Number(conn.chaptersOffset)).toString();
                  } else if (conn.chapters !== undefined) {
                    connChapters = conn.chapters.toString();
                  }

                  let connVolumes: string | undefined = undefined;
                  if (conn.volumesOffset !== undefined) {
                    connVolumes = (Number(data.volumes || 0) + Number(conn.volumesOffset)).toString();
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      if (isFavorited) {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/favorites/manga/${media.id}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${session.data.accessToken}`,
              ...
              if (val && typeof val === 'object') {
                acc[uppercaseKey] = {
                  ...
                  chaptersOffset: val.chapters !== undefined && val.chapters !== "" 
                    ? Number(val.chapters) - (chapters ? Number(chapters) : 0) 
                    : undefined,
                  volumesOffset: val.volumes !== undefined && val.volumes !== "" 
                    ? Number(val.volumes) - (volumes ? Number(volumes) : 0) 
                    : undefined,
  ...
  const toggleProgressOverride = (provider: string) => {
    setConnections((prev) => {
      ...
      const currentDetails = typeof current === 'object' ? current : {};
      const nextDetails = { ...currentDetails, id };
      if (typeof current === 'object' && (current.chapters !== undefined || current.volumes !== undefined)) {
        delete nextDetails.chapters;
        delete nextDetails.volumes;
      } else {
        nextDetails.chapters = chapters || "0";
        nextDetails.volumes = volumes || "0";
      }
      return { ...prev, [provider]: nextDetails };
    });
  };
  ...
  const handleChaptersOverrideChange = (provider: string, val: string) => {
    setConnections((prev) => {
      const current = prev[provider];
      const id = typeof current === 'object' ? current.id : current;
      const currentDetails = typeof current === 'object' ? current : {};
      return {
        ...prev,
        [provider]: { ...currentDetails, id, chapters: val },
      };
    });
  };

  const handleVolumesOverrideChange = (provider: string, val: string) => {
    setConnections((prev) => {
      const current = prev[provider];
      const id = typeof current === 'object' ? current.id : current;
      const currentDetails = typeof current === 'object' ? current : {};
  ...
  const renderConnectionCard = (provider: string, label: string) => {
    ...
    const connChapters = typeof conn === 'object' ? conn.chapters : undefined;
    const connVolumes = typeof conn === 'object' ? conn.volumes : undefined;
    ...
    const hasProgressOverride = typeof conn === 'object' && (conn.chapters !== undefined || conn.volumes !== undefined);
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center space-x-2">
                ...
                <Label
                  ...
                  className="text-xs font-medium text-muted-foreground cursor-pointer"
                >
                  Override progress (chapters / volumes)
                </Label>
              </div>
              {hasProgressOverride ? (
                <div className="grid grid-cols-2 gap-3 mt-1">
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Chapters</span>
                    <div className="flex bg-background border border-input rounded-md overflow-hidden h-9">
                      <Input
                        type="number"
                        min="0"
                        max={media.chapters || undefined}
                        value={connChapters !== undefined ? connChapters : ""}
                        onChange={(e) => handleChaptersOverrideChange(provider, e.target.value)}
                        className="border-0 bg-transparent text-foreground focus-visible:ring-0 h-full w-full px-3 text-xs"
                      />
                    </div>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Volumes</span>
                    <div className="flex bg-background border border-input rounded-md overflow-hidden h-9">
                      <Input
                        type="number"
                        min="0"
                        ...
                        onChange={(e) => handleVolumesOverrideChange(provider, e.target.value)}
                        className="border-0 bg-transparent text-foreground focus-visible:ring-0 h-full w-full px-3 text-xs"
                      />
                    </div>
                  </div>
                ...
                <span className="text-xs text-muted-foreground/80 pl-6 italic">
                  Inherited: Chapters: {chapters || "0"} / Volumes: {volumes || "0"}
                </span>
              )}
            </div>
