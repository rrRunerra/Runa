...
export class UpdateNotificationStatusDto {
  ...
  @IsIn(['PENDING', 'APPROVED', 'DENIED', 'READ'], {
    message:
      'UeNnSodto-SMBOTO001: Status must be one of PENDING, APPROVED, DENIED, READ',
  })
...
export class ApproveDeviceDto {
  ...
  @IsNotEmpty({
    message: 'AeDvdto-NIMNBE001: Notification id must not be empty',
  })
  notificationId!: string;

  @IsString({
    message: 'AeDvdto-EMKMBAS001: Encrypted master key must be a string',
  })
  @IsNotEmpty({
    message: 'AeDvdto-EMKMNBE001: Encrypted master key must not be empty',
  })
