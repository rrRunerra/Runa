...
export class ActorService {
  ...
  public async search(query: string): Promise<any[]> {
    ...
    const cacheKey = CacheService.keys.actorSearch(cleanName);
    ...
    if (cached && Array.isArray(cached)) {
      this.logger.debug(`Actor search cache hit for query: ${cleanName}`);
      return cached;
    }

    const data = await this.actorRepository.search(cleanName);
