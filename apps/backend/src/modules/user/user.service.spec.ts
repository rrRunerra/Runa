...
describe('UserService', () => {
  ...
  describe('create', () => {
    it('should throw ConflictException if username is reserved keyword', async () => {
      const dto = { username: 'class', email: 'test@runa.com', password: 'password123' };
      await expect(service.create(dto)).rejects.toThrow(ConflictException);
    });

    it('should throw ConflictException if email or username is already taken', async () => {
      const dto = { username: 'testuser', email: 'test@runa.com', password: 'password123' };
  ...
  describe('update', () => {
    it('should throw NotFoundException if user does not exist', async () => {
      ...
      await expect(service.update('user-1', { displayName: 'Name' })).rejects.toThrow(
        NotFoundException,
      );
    });

    it('should update display name and properties', async () => {
      const user = { id: 'user-1', username: 'testuser', email: 'test@runa.com' };
      mockPrismaClient.user.findUnique.mockResolvedValue(user);
      mockPrismaClient.user.update.mockResolvedValue({ ...user, displayName: 'New Display' });

      const result = await service.update('user-1', { displayName: 'New Display' });
    ...
    it('should verify current password and hash new password if updating password', async () => {
      const user = { id: 'user-1', username: 'testuser', passwordHash: 'old-hashed-pass' };
      mockPrismaClient.user.findUnique.mockResolvedValue(user);
      (bcrypt.compare as jest.Mock).mockResolvedValue(true);
      (bcrypt.hash as jest.Mock).mockResolvedValue('new-hashed-pass');
      mockPrismaClient.user.update.mockResolvedValue(user);

      await service.update('user-1', { currentPassword: 'oldpassword', newPassword: 'newpassword' });

      expect(bcrypt.compare).toHaveBeenCalledWith('oldpassword', 'old-hashed-pass');
    ...
    it('should throw BadRequestException if currentPassword is wrong', async () => {
      ...
      await expect(
        service.update('user-1', { currentPassword: 'wrongpassword', newPassword: 'newpassword' }),
    ...
    it('should delete old files if url changes', async () => {
      ...
      mockPrismaClient.user.findUnique.mockResolvedValue(user);
      mockPrismaClient.user.update.mockResolvedValue({ ...user, avatarUrl: 'new-avatar.png' });

      await service.update('user-1', { avatarUrl: 'new-avatar.png' });

      expect(mediaService.deleteFileByUrl).toHaveBeenCalledWith('old-avatar.png');
    });
  });
  ...
  describe('privacy settings', () => {
    ...
    it('should get parsed privacy settings', async () => {
      mockPrismaClient.user.findUnique.mockResolvedValue({ privacy: { animeList: true } });
    ...
    it('should update privacy settings and run transactional updates on list tables', async () => {
      mockPrismaClient.user.findUnique.mockResolvedValue({ id: 'user-1', username: 'testuser', privacy: {} });
      mockPrismaClient.$transaction.mockResolvedValue([]);

      await service.updatePrivacySettings('user-1', { animeList: true, profile: false });
  ...
  describe('TOTP MFA', () => {
    it('should generate TOTP setup secret and URI, caching secret', async () => {
      ...
      expect(cacheService.set).toHaveBeenCalledWith('pending-totp:user-1', 'totpsecret', 600);
      expect(result).toEqual({ secret: 'totpsecret', otpauthUrl: 'otpauth://totp/Runa' });
    });
  ...
  describe('Email MFA', () => {
    it('should send email mfa setup code', async () => {
      mockPrismaClient.user.findUnique.mockResolvedValue({ id: 'user-1', email: 'test@runa.com' });
      const result = await service.sendEmailMfaSetupCode('user-1');

      expect(cacheService.set).toHaveBeenCalledWith('pending-email-mfa:user-1', expect.any(String), 300);
  ...
  describe('Passkeys / WebAuthn', () => {
    it('should generate registration options', async () => {
      ...
      (generateRegistrationOptions as jest.Mock).mockResolvedValue({ challenge: 'webauthn-challenge' });
    ...
    it('should delete passkey and clean up backup codes if no MFA left', async () => {
      ...
      expect(mockPrismaClient.passkey.delete).toHaveBeenCalledWith({ where: { id: 'pass-1' } });
      expect(mockPrismaClient.user.update).toHaveBeenCalledWith({
        where: { id: 'user-1' },
        data: { backupCodes: [] },
      });
