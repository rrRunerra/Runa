...
function decryptDataKeyForUser(
  ...
  const ephemeralPublicKeyBuf = Buffer.from(
...
function decryptBufferWithDataKey(
...
describe('E2E Crypto Utilities', () => {
  ...
  it('should perform hybrid ECDH key agreement to wrap and unwrap a data key', () => {
    ...
    const wrappedPayload = encryptDataKeyForUser(
      clientPublicKeyBase64Url,
      dataKey,
    );
    ...
    const unwrappedDataKey = decryptDataKeyForUser(
      wrappedPayload,
      clientPrivateKey,
    );
