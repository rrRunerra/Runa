...
async function bootstrap() {
  ...
  if (!username) {
    console.error(
      'Error: Please specify a username. Usage: pnpm run db:assign-admin <username>',
    );
    process.exit(1);
  }

  console.log(
    `--- Starting Admin Assignment Script for user: "${username}" ---`,
  );
  ...
  try {
    ...
    if (permissions.has(RunaFlags.ADMINISTRATOR)) {
      ...
      console.log(
        `Successfully assigned ADMINISTRATOR to user "${user.username}"!`,
      );
