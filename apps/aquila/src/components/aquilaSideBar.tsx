...
export default function AquilaSideBar({
  ...
  const aquilaNavConfig = useMemo(
    () => getAquilaSidebarConfig(session, connections),
    [session, connections],
  );

  const { navbarConfig, setNavbarConfig, insertSection, getSection } =
    useNavigation();

  // Handle base configuration sync manually to allow runtime persistence
  useEffect(() => {
    setNavbarConfig((prev: NavbarConfig) => {
      // If empty, just initialize
      if (prev.length === 0) return aquilaNavConfig;

      // Smart merge: Update base sections while preserving dynamic ones
      const next = [...prev];
      aquilaNavConfig.forEach((baseSection) => {
      ...
      // Remove base sections that no longer exist (e.g. logout)
      return next.filter(
        (s) =>
          !getAquilaSidebarConfig(session, connections).some(
            (b) => b.section === s.section,
          ) || aquilaNavConfig.some((b) => b.section === s.section),
      ) as NavbarConfig;
    });
  }, [aquilaNavConfig, setNavbarConfig, session, connections]);
