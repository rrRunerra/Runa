...
import { LinkIcon, Unlink, ExternalLink, CheckCircle2, AlertCircle, RefreshCw } from "lucide-react";
...
export default function ConnectionsPage() {
  ...
  useEffect(() => {
    ...
    if (err) {
      setError(err === "oauth_failed" ? "Authentication with the third-party app failed." : "An error occurred.");
      router.replace("/connections");
    }
  }, [searchParams, router]);
  ...
  const handleConnect = (providerId: string) => {
    ...
    window.location.href = `/api/connections/${providerId}/connect`;
  };
  ...
  const handleDisconnect = async (providerId: string) => {
    ...
    setIsActionLoading(providerId);
    try {
      const res = await fetch(`/api/connections?provider=${providerId.toUpperCase()}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error("Failed to disconnect");
      
      setConnections((prev) => prev.filter((c) => c.provider.toLowerCase() !== providerId.toLowerCase()));
      setSuccess(true);
  ...
  if (status === "loading" || (status === "authenticated" && isLoading)) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <p className="text-muted-foreground animate-pulse">Loading your connections...</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto p-6 md:p-10 max-w-5xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {PROVIDERS.map((provider) => {
          ...
          return (
            <Card key={provider.id} className="group flex flex-col h-full bg-card/40 backdrop-blur-sm border-border/50 hover:border-primary/30 transition-all duration-300">
              <CardHeader className="pb-4">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    provider.id === "anilist" ? "bg-[#3db4f2] p-2" : "bg-transparent"
                  )}>
                    <Image
                      src={provider.icon}
                      alt={provider.name}
                ...
                <CardAction>
                  {isConnected ? (
                    <Badge variant="secondary" className="rounded-full bg-primary/15 text-primary border-primary/10 font-bold px-3 py-1">
                      Connected
                    </Badge>
                  ) : (
                    <Badge variant="outline" className="rounded-full text-muted-foreground/60 border-border/40 font-bold px-3 py-1">
                      Disconnected
                    </Badge>
                  )}
                </CardAction>
                ...
                {isConnected ? (
                  <div className="p-3 rounded-lg bg-background/50 border border-border/40 space-y-1.5 animate-in fade-in duration-300">
                    <div className="flex items-center justify-between overflow-hidden">
                      ...
                      <span className="text-sm font-semibold truncate ml-2">{conn.username}</span>
                    </div>
                    {conn.createdAt && (
              ...
              <CardFooter className="pt-4 pb-6 flex gap-2 mt-auto">
                {isConnected ? (
                  <>
                    ...
                    >
                      {loading ? <Spinner className="h-4 w-4" /> : <Unlink className="h-4 w-4" />}
                  ...
                  >
                    {loading ? <Spinner className="h-4 w-4" /> : <LinkIcon className="h-4 w-4" />}
      ...
      <div className="mt-12 p-8 rounded-3xl bg-secondary/10 border border-border/30 text-center space-y-4">
        ...
        <p className="text-muted-foreground text-sm max-w-sm mx-auto">
          If your watch history isn't updating correctly, try refreshing your connection or reconnecting your account.
        </p>
        <div className="pt-2">
          <Button variant="outline" size="sm" className="rounded-full px-6 hover:bg-primary/5 border-primary/20" onClick={fetchConnections}>
