...
| [caveman](./../caveman/SKILL.md) | Always active — use when responding to user |
...
### Data & Hooks
- **Never** use native `fetch` — always use `useFetch`. If `useFetch` can't do what you need, **stop and ask the user**.
- **Always** use `useRRSidebar` from `@/hooks/useRRSidebar` for sidebar control — not the shadcn `useSidebar`.
